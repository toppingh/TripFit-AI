...
from app.services.filtering_service import get_place_data, filtering_places
from app.services.gemini_service import trip_course_create
from app.services.tour_api_service import get_area_places
...
async def recommend(req: RecommendRequest):
    ...
    data = get_area_places(req.areaCode)

    places = get_place_data(data)
    result = filtering_places(places)

    return {
        "tour_spots": result["tours"][:5],
        "eat_spots": result["eats"][:5],
        "hotel_spots": result["hotels"][:5],
        "festival_spots": result["festivals"][:5],
    }
